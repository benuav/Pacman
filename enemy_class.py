...
class Enemy:
    ...
    def update(self):
        ...
        if self.target != self.grid_pos:
            self.pix_pos += self.direction * self.speed  # get direction into pix position
            if self.time_to_move(): # check walls
                self.move()# stop or move

        # Setting grid position in reference to pix position
        self.grid_pos[0] = (self.pix_pos[0]-TP_BUFFER +
                            self.app.cell_width//2)//self.app.cell_width+1
        self.grid_pos[1] = (self.pix_pos[1]-TP_BUFFER +
